...
public class KospiKosdaqSwitch extends SuperFinance implements UrlFactory
{
	...
	public Object excute(FinanceDataMatrix financeDataMatrix)
	{
		...
		for (int i = 0; i < indexSub.length; i++)
			outMap.put(super.market + indexSub[i], super.parsingContainer[i]);
		...
		for (int i = 0; i < super.contents.size(); i++)
			outMap.put(super.market + isub[i], super.contents.get(i).attr("src"));
		kosdaqArr.add((Map<String, String>) outMap.clone());
		/* 차트 이미지 종료 */
		return kosdaqArr;
	}
